IF OBJECT_ID('[IcsApp-Tests].[test StartRun calls ExceptionHandler on ics.ProcessRunStart error]') IS NOT NULL
	DROP PROCEDURE [IcsApp-Tests].[test StartRun calls ExceptionHandler on ics.ProcessRunStart error];
