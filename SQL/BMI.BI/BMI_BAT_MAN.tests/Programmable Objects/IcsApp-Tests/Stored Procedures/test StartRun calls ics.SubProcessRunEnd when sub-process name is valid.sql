IF OBJECT_ID('[IcsApp-Tests].[test StartRun calls ics.SubProcessRunEnd when sub-process name is valid]') IS NOT NULL
	DROP PROCEDURE [IcsApp-Tests].[test StartRun calls ics.SubProcessRunEnd when sub-process name is valid];
