IF OBJECT_ID('[IcsApp-Tests].[test StartRun throws error on @ProcessRunId zero when starting a sub-process]') IS NOT NULL
	DROP PROCEDURE [IcsApp-Tests].[test StartRun throws error on @ProcessRunId zero when starting a sub-process];
...
SET ANSI_NULLS ON
GO
create   procedure [IcsApp-Tests].[test StartRun throws error on @ProcessRunId zero when starting a sub-process]
as
begin
