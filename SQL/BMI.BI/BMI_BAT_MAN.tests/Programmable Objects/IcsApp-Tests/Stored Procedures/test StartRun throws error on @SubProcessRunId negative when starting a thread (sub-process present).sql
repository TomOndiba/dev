IF OBJECT_ID('[IcsApp-Tests].[test StartRun throws error on @SubProcessRunId negative when starting a thread (sub-process present)]') IS NOT NULL
	DROP PROCEDURE [IcsApp-Tests].[test StartRun throws error on @SubProcessRunId negative when starting a thread (sub-process present)];
