IF OBJECT_ID('[IcsApp-Tests].[test StartRun throws error on @SubProcessRunId null when starting a thread (sub-process missing)]') IS NOT NULL
	DROP PROCEDURE [IcsApp-Tests].[test StartRun throws error on @SubProcessRunId null when starting a thread (sub-process missing)];
