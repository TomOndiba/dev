IF OBJECT_ID('[IcsApp-Tests].[test StopRun throws error on @FailedSourceRows negative when ending a thread]') IS NOT NULL
	DROP PROCEDURE [IcsApp-Tests].[test StopRun throws error on @FailedSourceRows negative when ending a thread];
