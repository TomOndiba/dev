IF OBJECT_ID('[IcsApp-Tests].[test StopRun throws error on @ProcessRunId null when ending a process]') IS NOT NULL
	DROP PROCEDURE [IcsApp-Tests].[test StopRun throws error on @ProcessRunId null when ending a process];
...
SET ANSI_NULLS ON
GO
create   procedure [IcsApp-Tests].[test StopRun throws error on @ProcessRunId null when ending a process]
as
begin
