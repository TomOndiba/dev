IF OBJECT_ID('[IcsApp-Tests].[test StopRun throws error on @ProcessRunId zero when ending a process]') IS NOT NULL
	DROP PROCEDURE [IcsApp-Tests].[test StopRun throws error on @ProcessRunId zero when ending a process];
