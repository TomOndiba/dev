IF OBJECT_ID('[IcsApp-Tests].[test StopRun throws error on @SubProcessRunId null when ending a sub-process (process present)]') IS NOT NULL
	DROP PROCEDURE [IcsApp-Tests].[test StopRun throws error on @SubProcessRunId null when ending a sub-process (process present)];
