IF OBJECT_ID('[IcsApp-Tests].[test StopRun throws error on @SubProcessRunId zero when ending a sub-process (process not present)]') IS NOT NULL
	DROP PROCEDURE [IcsApp-Tests].[test StopRun throws error on @SubProcessRunId zero when ending a sub-process (process not present)];
