IF OBJECT_ID('[IcsApp-Tests].[test StopRun throws error on @ThreadRunId negative when ending a thread (sub-process present)]') IS NOT NULL
	DROP PROCEDURE [IcsApp-Tests].[test StopRun throws error on @ThreadRunId negative when ending a thread (sub-process present)];
