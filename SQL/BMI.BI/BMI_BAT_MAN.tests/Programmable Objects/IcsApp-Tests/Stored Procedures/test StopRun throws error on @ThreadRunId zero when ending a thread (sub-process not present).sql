IF OBJECT_ID('[IcsApp-Tests].[test StopRun throws error on @ThreadRunId zero when ending a thread (sub-process not present)]') IS NOT NULL
	DROP PROCEDURE [IcsApp-Tests].[test StopRun throws error on @ThreadRunId zero when ending a thread (sub-process not present)];
