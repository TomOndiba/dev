IF OBJECT_ID('[IcsApp-Tests].[test StopRun throws error on @ThreadRunId zero when ending a thread (sub-process present)]') IS NOT NULL
	DROP PROCEDURE [IcsApp-Tests].[test StopRun throws error on @ThreadRunId zero when ending a thread (sub-process present)];
...
SET ANSI_NULLS ON
GO
create   procedure [IcsApp-Tests].[test StopRun throws error on @ThreadRunId zero when ending a thread (sub-process present)]
as
begin
