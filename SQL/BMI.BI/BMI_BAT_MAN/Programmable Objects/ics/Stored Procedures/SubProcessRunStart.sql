	...
	begin
		...
		begin try
			...
			exec [batch].[ProcessGetIdFromIcrtProcessName]
			  ...
			  , @BatchProcessName = null
			  , @BatchProcessId = @_BatchProcessId output ;
		...
		begin catch
			set @_ErrorContext = 'Failed to start new sub-process run' + ' for ICRT Process: ' + coalesce('"' + @ProcessName + '"', 'NULL')
								 + ' , ICRT Sub-Process: ' + coalesce('"' + @SubProcessName + '"', 'NULL') + ' and (BatMan) Process Run Id: '
								 + coalesce(cast(@ProcessRunId as varchar(32)), 'NULL') + ' at step: ' + coalesce('[' + @_Step + ']', 'NULL')
