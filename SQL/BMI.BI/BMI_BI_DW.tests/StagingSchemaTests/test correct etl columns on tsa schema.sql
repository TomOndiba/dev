...
begin
	...
	as
	(
				  select 'EtlBatchRunId'	, null		, 'NO'	, 'int'			, null	, 10	, 0
		...
		union all select 'ExcludeFromMerge'	, '((0))'	, 'NO'	, 'bit'			, null	, null	, null
		union all select 'IsDuplicate'		, '((0))'	, 'NO'	, 'bit'			, null	, null	, null
	)
	, expectedCte
	as
	(
		...
		where
				t.TABLE_SCHEMA = 'tsa'
			and t.TABLE_NAME like 'ics_land_%'
	)
	, actualCte
	as
	(
		...
		where
				ac.TABLE_SCHEMA = 'tsa'
			and ac.TABLE_NAME like 'ics_land_%'
	...
	declare @_FailMsg nvarchar(500) = 'One or more ETL Audit columns are not correctly defined on one or more TSA tables.' + char(13) + char(10) ;
