CREATE      procedure [tsa-to-psa-tests].[test privy.BuildAndRunMerge correctly updates null time columns on update]
as
	begin

		declare @_now varchar(200) = getdate() 
		,@_time time =cast(getdate() as time);
		...
		select	1, 1, 1, @_now, 'Razia', 'Dummy', 1, 1, @_time;
