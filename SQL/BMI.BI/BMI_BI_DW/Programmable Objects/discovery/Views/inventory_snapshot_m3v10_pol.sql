...
select
  ...
  , MITTRA.EtlUpdatedOn									  as etl_updated_on
  , MITTRA.EtlSourceTable								  as etl_source_table
  , MITTRA.IsDeleted									  as is_deleted
  , max(cast(cast(MITTRA.MTTRDT as char(8)) as datetime)) as snapshot_date
...
select
  ...
  , MITTRA.EtlUpdatedOn									  as etl_updated_on
  , MITTRA.EtlSourceTable								  as etl_source_table
  , MITTRA.IsDeleted									  as is_deletede
  , max(cast(cast(MITTRA.MTTRDT as char(8)) as datetime)) as transaction_date
  , MITTRA.DataSourceKey								  as data_source_key
  , ds.DataSourceName									  as data_source_name
  , 'POL'												  as country_code
  , MITTRA.MTWHLO										  as native_warehouse_number
