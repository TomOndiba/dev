...
(
	select
	  ...
	  , A.MUDMCF
	from
		psa.ics_stg_M3V10vil_POL_MITAUN		A
	inner join psa.ics_stg_M3V10vil_POL_MITMAS B
  ...
  , cast(case
			 when upper(l.IBPUUN) in
				 ...
				 then l.IBRJQA
					  ...
					  when (upper(l.IBPUUN) = 'KG'and upper(l.IBPUUN) <> 'KG')
						  then (l.IBRJQA / 1000)
					  when (upper(alternative) = 'KG')
						  then ((l.IBRJQA / conversion) / 1000)
					  else (l.IBRJQA / conversion)
...
(
	select
	  ...
	  , A.MUDMCF
	from
		psa.ics_stg_M3V10ved_POL_MITAUN		A
	inner join psa.ics_stg_M3V10ved_POL_MITMAS B
		on (A.MUITNO = B.MMITNO)
			and A.MUAUTP = 1
			and upper(A.MUALUN) in
					('KG', 'M2', 'TN')
...
(
	select
	  ...
	  , A.MUDMCF
	from
		psa.ics_stg_M3V10gen_POL_MITAUN		A
	inner join psa.ics_stg_M3V10gen_POL_MITMAS B
		on (A.MUITNO = B.MMITNO)
			and A.MUAUTP = 1
			and upper(A.MUALUN) in
					('KG', 'M2', 'TN')
