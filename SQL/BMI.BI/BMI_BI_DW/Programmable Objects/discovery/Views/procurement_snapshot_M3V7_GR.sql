IF OBJECT_ID('[discovery].[procurement_snapshot_M3V7_GR]') IS NOT NULL
	DROP VIEW [discovery].[procurement_snapshot_M3V7_GR];
...
create      view [discovery].[procurement_snapshot_M3V7_GR]
as
select
--count(*)
 ...
 , case when IAWHLO in ('14', '16') then 'Spain' else 'France' end as DataSource
  , g.DataSourceKey
  , (
		select	max(d.EtlCreatedOn) from psa.ics_stg_m3v7_FGRECL d
	)									  as DateDataExtracted
from
	psa.ics_stg_m3v7_FGRECL			  g
left outer join
(
	select
		B.MMITNO
	  ...
	  , A.MUDMCF
	from
		psa.ics_stg_m3v7_MITMAS			  B
	left outer join psa.ics_stg_m3v7_MITAUN A
...
left outer join psa.ics_stg_m3v7_MPHEAD h
