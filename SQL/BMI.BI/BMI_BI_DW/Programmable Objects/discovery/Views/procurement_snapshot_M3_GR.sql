IF OBJECT_ID('[discovery].[procurement_snapshot_M3_GR]') IS NOT NULL
	DROP VIEW [discovery].[procurement_snapshot_M3_GR];
...
create     view [discovery].[procurement_snapshot_M3_GR]
as
select
cc.alternative,
cast(h.IAFACI as nvarchar(255))		as Facility--								  as Facility
  ...
  , g.DataSourceKey
  , (
		select	max(d.EtlCreatedOn) from psa.ics_stg_m3_FGRECL d
	)									  as DateDataExtracted
from
	psa.ics_stg_m3_FGRECL g
left outer join
(
	select
		B.MMITNO
	  ...
	  , A.MUAUTP
	  , A.MUDMCF
	from psa.ics_stg_m3_MITMAS B
		
	left outer join psa.ics_stg_m3_MITAUN		 A
...
(
	...
	left outer join 	psa.ics_stg_m3_MPHEAD	 h
	on h.IAPUNO=g.F2PUNO
where
	year(cast(g.F2TRDT as nvarchar(20))) > '2016'
	and g.F2RPQT <> '0'
	and g.F2RPQA <> '0'
	and g.F2IVQT <> '0'
	and g.F2IVQA <> '0' ;
