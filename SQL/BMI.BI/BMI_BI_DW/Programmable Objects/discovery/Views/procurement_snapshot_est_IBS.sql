IF OBJECT_ID('[discovery].[procurement_snapshot_est_IBS]') IS NOT NULL
	DROP VIEW [discovery].[procurement_snapshot_est_IBS];
...
create    view [discovery].[procurement_snapshot_est_IBS]
as
select
	OHDIVN								  as [Division]
  , 'B3'								  [CompanyNumber]
  ...
  , (
		select	max(d.EtlCreatedOn) from psa.ics_stg_ibs_est_procure_SROORPHE d
	)									  [DateDataExtracted]
from
	psa.ics_stg_ibs_est_procure_SROORPHE			  h
left outer join psa.ics_stg_ibs_est_procure_SROORPPL l
	on h.OHORNO = l.OLORNO
left outer join psa.ics_stg_ibs_est_procure_SROGRT	  g
	on g.GRORNO = l.OLORNO
		and g.GRPRDC = l.OLPRDC
		and g.GRLINE = l.OLLINE
left outer join psa.ics_stg_ibs_est_procure_SROPRU	  u
