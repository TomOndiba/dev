IF OBJECT_ID('[discovery].[procurement_snapshot_fin_IBS]') IS NOT NULL
	DROP VIEW [discovery].[procurement_snapshot_fin_IBS];
...
create view [discovery].[procurement_snapshot_fin_IBS]
as
select
	OHDIVN								  as [Division]
  , '01'								  [CompanyNumber]
  ...
  , (
		select	max(d.EtlCreatedOn) from psa.ics_stg_ibs_fin_procure_SROORPHE d
	)									  [DateDataExtracted]
from
	psa.ics_stg_ibs_fin_procure_SROORPHE			  h
left outer join psa.ics_stg_ibs_fin_procure_SROORPPL l
	on h.OHORNO = l.OLORNO
left outer join psa.ics_stg_ibs_fin_procure_SROGRT	  g
