...
merge into [etl].[unit_of_measure_lookup] as tgt
using (values
  (100101,'B1','',44)
 ,(100101,'BAG','',45)
 ,(100101,'BD','',46)
...
on (tgt.[data_source_key] = Source.[data_source_key] and tgt.[native_key] = Source.[native_key])
when matched and (
	nullif(Source.[native_value], tgt.[native_value]) is not null or nullif(tgt.[native_value], Source.[native_value]) is not null or 
	nullif(Source.[unit_of_measure_id], tgt.[unit_of_measure_id]) is not null or nullif(tgt.[unit_of_measure_id], Source.[unit_of_measure_id]) is not null) then
 update set
