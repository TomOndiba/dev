...
CREATE view [dbo].[V_SA_INVOICE_TEST_delete] as 
...
from sa_INVOICE as inv with (nolock)
       left outer join
         PU_LINK_PAYMENTTERM as cpay with (nolock)
                                     on (cpay.SYSTEM_ID     = inv.SYSTEM_ID and
                                         cpay.PAYMENT_TERM  = inv.PAYMENT_TERM)
       join
         PU_LINK_SITE as ssite with (nolock) on (ssite.SYSTEM_ID  = inv.SYSTEM_ID and
	   ...
	   left outer join
         PU_LINK_UOM as plu with (nolock) on (plu.SYSTEM_ID = inv.SYSTEM_ID and
                                plu.LOCAL_UOM = inv.UOM)
	   left outer join
	     SA_LINK_ITEM as lic with (nolock) on (lic.SYSTEM_ID = inv.SYSTEM_ID and
		                         lic.ITEM_NO   = inv.ITEM_NO)
	   left outer join
	     SA_LINK_ENV as lec with (nolock) on (lec.SYSTEM_ID = inv.SYSTEM_ID and
		                        lec.ITEM_NO   = inv.ITEM_NO)
	   left outer join
	     SA_LINK_CUSTOMER as lis with (nolock) on (lis.SYSTEM_ID   = inv.SYSTEM_ID and
