		...
		if left(@@version, 19) = 'Microsoft SQL Azure'
			...
			begin
				...
				exec sys.sp_executesql
					...
					, @NTDomainOUT = @NTDomain
					, @NTUsernameOUT = @NTUsername
					, @LoginNameOUT = @LoginName
					, @OriginalLoginNameOUT = @OriginalLoginName
					, @SessionLoginTimeOUT = @SessionLoginTime
			end
	end try
	begin catch
...
go
grant execute on  [log4Utils].[ExceptionReader] to [public]
go
...
go
grant execute on  [log4Utils].[JournalReader] to [public]
go
...
go
grant execute on  [log4Utils].[PrintString] to [public]
go
