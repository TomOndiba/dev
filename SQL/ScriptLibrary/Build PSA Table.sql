 ...
 * 
 * USAGE:
 * call from SQLCMD passing in the base table, data source and a flag to run in test rather than commit mode e.g.
 * SQLCMD -S localhost -E -d BMI_BI_DW -i "Build PSA Table.sql" -v sourceTsaTable=M3V10gen_POL_MWOPOL -v dataSourceCode=Nav-BGR -v doNotCommit=0
 *
 * To create multiple tables, create a list in Excel/Sheets and use the CONCATINATE function to build a list of SQLCMD commands, then paste them into a .bat
 *
...
DECLARE @sourceTsaTable NVARCHAR(128) = N'$(sourceTsaTable)' -- N'ics_land_M3V10gen_POL_MWOPOL';
DECLARE @dataSourceCode NVARCHAR(20) = N'$(dataSourceCode)' -- N'Nav-BGR';
DECLARE @doNotCommit BIT = $(doNotCommit);
